...
public class DomesticInfoController {
    ...
    public Map<String, DomesticInfo> getDomesticCovidInfo() {
        ...
        DomesticInfo todayInfo = domesticInfoMapper.selectMostRecentInfoByDay(today);	// 오늘 국내 정보 조회 (api를 주기적으로 요청하여 데이터베이스에 저장할 때, 해당 날짜의 모든 정보를 지우고 insert 하기 때문에  최신 데이터를 조회할 필요 없음.)
        DomesticInfo yesterdayInfo = domesticInfoMapper.selectMostRecentInfoByDay(yesterday);	// 어제 국내 정보 조회
